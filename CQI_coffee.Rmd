...
for (dir in list.dirs("utils")){
  for (file in list.files(dir, pattern="*.R", full.names = TRUE, include.dirs = TRUE)){
    source(file)
    message(paste0("Imported: ", file))
  }
}
```
...
```{r}
print_colnames(raw_arabica, message = FALSE)
print_colnames(raw_robusta, message = FALSE)
```
...
dtypes <- sapply(merged_df, class, simplify = T)

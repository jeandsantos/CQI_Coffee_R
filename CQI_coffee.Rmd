...
if(print_plots){ custom_visdat(merged_df, save_plot = save_plots, save_dir = save_plots_dir) }
...
altitude_text <- merged_df$ALTITUDE %>% unique() %>% 
    stringr::str_to_lower() %>% 
    { if(rm_accents) stringi::stri_trans_general(., accents_to) else .} %>% 
    { if(rm_redundant) stringr::str_remove_all(., redundant_regex) else .} %>% 
    { if(rm_punct) stringr::str_remove_all(., punct_regex) else .} %>%
...
(altitude_numeric <- merged_df$ALTITUDE %>% unique() %>% 
    { if(rm_punct) stringr::str_remove_all(., punct_regex) else .} %>%
...
altitude_numeric <- as.numeric(as.matrix(altitude_numeric))

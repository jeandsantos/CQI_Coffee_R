...
data.frame(altitude_numeric, altitude_text)
```


```{r}
altitude_text %>% unique()
```
...
```{r}
altitude_text %>% unique() %>% str_extract_all(., pattern = ft_regex, simplify = TRUE)
```


```{r}
custom_visdat(altitude_numeric)
```
